...
import {
  LayoutDashboard, DollarSign, MessageSquare, Activity,
  ChevronLeft, ChevronRight, LogOut, Menu, X
} from "lucide-react";

const sections = [
  { label: "Overview", icon: LayoutDashboard, path: "/agent" },
  { label: "Funding Requests", icon: DollarSign, path: "/agent/funding" },
  { label: "Chat", icon: MessageSquare, path: "/agent/chat" },
  { label: "Activity", icon: Activity, path: "/agent/activity" },
];
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <aside className={`fixed top-0 left-0 h-full z-50 bg-card border-r border-border flex flex-col transition-all duration-300
        ...
        <div className="flex items-center gap-2 p-4 border-b border-border h-16">
          <div className="w-8 h-8 rounded-full bg-accent/20 flex items-center justify-center text-xs font-bold text-accent flex-shrink-0">A</div>
          {!collapsed && <span className="font-display font-bold text-foreground text-sm">Agent Panel</span>}
      ...
      <div className={`flex-1 transition-all duration-300 ${collapsed ? "md:ml-16" : "md:ml-56"}`}>
        <header className="sticky top-0 z-30 bg-card/80 backdrop-blur-xl border-b border-border h-14 flex items-center px-4 gap-3">
          ...
          <h2 className="font-display font-semibold text-foreground text-sm">
            {sections.find(s => s.path === location.pathname)?.label || "Agent"}
          </h2>
